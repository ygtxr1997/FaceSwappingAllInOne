...
def paste_back(swapped: Image.Image, target_ori: Image.Image, target_affine_m: np.ndarray,
               img_mask=None, trick=None, use_post=False):
    """ Paste img to ori_img """
    img, mat, ori_img = np.array(swapped), target_affine_m, np.array(target_ori)
    if mat is None:
        print('[Warning] mat is None.')
        ori_img = ori_img.astype(np.uint8)
