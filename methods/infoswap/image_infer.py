...
class InfoSwapImageInfer(torch.nn.Module):
    def __init__(self):
        super(InfoSwapImageInfer, self).__init__()
        self.ib_mode = 'smooth'
        self.N = 10
        self.G = None
        self.iib = None
    ...
    def load_model(self):
        ROOT = {
            'smooth': {'root': make_abs_path('../weights/InfoSwap/weights/checkpoints_512/w_kernel_smooth'),
                       'path': 'ckpt_ks_*.pth'},
            'no_smooth': {'root': make_abs_path('../weights/InfoSwap/weights/checkpoints_512/wo_kernel_smooth'),
                          'path': 'ckpt_*.pth'}
        }
        ...
        state_dict = torch.load(make_abs_path('../weights/InfoSwap/weights/model_128_ir_se50.pth'),
                                map_location=device)
    ...
    def infer_batch(self, source_batch, target_batch):
        ...
        for i in range(self.N + 1):
            state = torch.load(make_abs_path(f'../weights/InfoSwap/weights128/readout_layer{i}.pth'),
                               map_location=device)
...
if __name__ == '__main__':
    ...
    infoswap_model = InfoSwapImageInfer()
