...
class StyleFusionSimple:
    ...
    def generate_img(self, base_latent, latents_type="z", hair=None, face=None, background=None, all=None, mouth=None,
        ...
        swap(hair, ["bg_hair_clothes", "hair"])  # target
        swap(face, ["face", "eyes", "skin_mouth", "mouth", "skin", "shirt"])  # source
        swap(background, ["background", "background_top", "background_bottom", "bg"])  # target
        swap(all, ["all"])
        swap(mouth, ["skin_mouth", "face"])  # source
        swap(eyes, ["eyes", "face"])  # source
        swap(wheels, ["wheels"])
