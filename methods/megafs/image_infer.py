...
from .inference.megafs import resnet50, HieRFE, Generator, FaceTransferModule
...
class MegaFSImageInfer(torch.nn.Module):
    def __init__(self, swap_type: str = 'ftm'):
        ...
        ckpt_e = make_abs_path("../weights/MegaFS/inference/checkpoint/{}_final.pth".format(self.swap_type))
        ...
        ckpt_f = make_abs_path("../weights/MegaFS/inference/checkpoint/stylegan2-ffhq-config-f.pth")
    ...
    def forward(self, source, target):
        return self.infer_batch(source, target)
    ...
    def infer_image(self, source: Image.Image, target: Image.Image, **kwargs):
        ...
        s = pil_to_tensor(source.resize((256, 256)))
        t = pil_to_tensor(target.resize((256, 256)))
        r = self.forward(s, t)
